...
class ChildProgressDetailScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (results.isEmpty)
            Text(
              t ? 'এখনও কোনো কুইজ সম্পন্ন হয়নি।' : 'No quizzes completed yet.',
          ...
          Text(
            t ? 'অর্জিত ব্যাজ' : 'Badges Earned',
            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 8),
          if (badges.isEmpty)
            Text(
              t ? 'এখনও কোনো ব্যাজ অর্জিত হয়নি।' : 'No badges earned yet.',
              style: const TextStyle(color: Colors.grey),
            )
          else
            Wrap(
  ...
  String _moduleTitleForQuiz(String quizId, bool bengali) {
    final quiz = MockData.quizzesByModule.values.firstWhere(
      (q) => q.id == quizId,
    );
