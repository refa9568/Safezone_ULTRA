      ...
      child: Stack(
        ...
        children: [
          CustomPaint(
            ...
            painter: _SafetyMascotPainter(
              bodyColor: bodyColor,
              variant: MascotVariant.values[variant],
            ),
          ),
          Positioned(
            top: height * 0.42,
  ...
  Path _bodyPath(MascotVariant variant, double w, double h) {
    switch (variant) {
      ...
      case MascotVariant.robot:
        ...
        return Path()
          ..addRRect(RRect.fromRectAndRadius(rect, Radius.circular(w * 0.22)));
  ...
  void _drawFace(Canvas canvas, MascotVariant variant, double w, double h) {
    ...
    switch (variant) {
      ...
      case MascotVariant.shield:
        ...
        for (final dx in [-1.0, 1.0]) {
          ...
          canvas.drawOval(
            Rect.fromCenter(center: center, width: w * 0.15, height: h * 0.12),
            eyeWhite,
          );
          canvas.drawCircle(
            Offset(center.dx + dx * 1.5, center.dy + 1.5),
            w * 0.035,
            pupil,
          );
          canvas.drawCircle(
            Offset(center.dx + dx * 1.5 - 1.5, center.dy - 1.5),
            w * 0.012,
            eyeShine,
          );
        }
        ...
        for (final dx in [-1.0, 1.0]) {
          ...
          canvas.drawRRect(
            RRect.fromRectAndRadius(
              Rect.fromCenter(center: center, width: w * 0.13, height: h * 0.1),
    ...
    switch (variant) {
      ...
      case MascotVariant.robot:
        // Flat neutral mouth
        canvas.drawLine(
          Offset(w * 0.44, mouthY),
          Offset(w * 0.56, mouthY),
          linePaint,
        );
      case MascotVariant.blob:
      case MascotVariant.cloud:
